...
export function CreateGraphForm({ closeModal, refreshGraphs }: JSONObject) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitHandler)} className={`${styles["modal-form"]} px-8`}>
      ...
      <InputToggleSwitch label="Enable Guide" className="mt-4" control={control} name={"enableGraphGuide"} description="Click the toggle and get a step-by-step tour on how to perform GraphOwls investigations" />
